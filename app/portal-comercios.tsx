...
type Section = "perfil" | "catalogo" | "ofertas";
...
function isProfileComplete(state: MerchantPortalState) {
  ...
  return Boolean(profile.name && profile.category && profile.address && profile.city && profile.phone && profile.schedule);
}
...
export default function MerchantPortalScreen() {
  ...
  const checklist = [
    ...
    { label: "Verificación de Google Maps", ready: false },
  ];
  ...
  const previewDiscount = previewOriginal > 0 && previewOffer > 0 ? Math.max(0, Math.round((1 - previewOffer / previewOriginal) * 100)) : 0;
  ...
  const saveProfile = async () => {
    if (!state.profile.name.trim() || !state.profile.address.trim() || !state.profile.city.trim() || !state.profile.phone.trim() || !state.profile.schedule.trim()) {
  ...
  const addProduct = async () => {
    ...
    if (!productName.trim() || !Number.isFinite(regularPrice) || regularPrice <= 0) {
      setNotice("Ingresá un producto y un precio válido para incorporarlo al catálogo.");
      return;
    }
  ...
  const chooseProduct = (product: MerchantCatalogItem) => {
    ...
    setNotice(`Preparando una oferta para ${product.name}.`);
  };

  const publishOffer = async () => {
    if (!selectedProduct || previewOriginal <= 0 || previewOffer <= 0 || previewOffer >= previewOriginal) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(offerStarts) || !/^\d{4}-\d{2}-\d{2}$/.test(offerEnds) || offerEnds < offerStarts) {
      setNotice("Usá fechas válidas con formato AAAA-MM-DD y una fecha final posterior a la inicial.");
      return;
    }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        {section === "perfil" ? (
          <View style={[styles.panel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Field label="Rubro" value={state.profile.category} onChangeText={(value) => setState((current) => ({ ...current, profile: { ...current.profile, category: value } }))} placeholder="Supermercado, mayorista..." colors={colors} />
            ...
            <Pressable onPress={() => void openGoogleBusiness()} style={({ pressed }) => [styles.googleHelp, { borderColor: colors.border }, pressed && styles.pressed]}>
              <Text style={[styles.googleHelpTitle, { color: colors.foreground }]}>Google Maps: iniciar verificación</Text>
        ...
        {section === "catalogo" ? (
          <View style={[styles.panel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Field label="Categoría" value={productCategory} onChangeText={setProductCategory} placeholder="Almacén" colors={colors} />
            ...
            {state.catalog.map((item) => (
              <View key={item.id} style={[styles.itemCard, { borderColor: colors.border }]}>
                <View style={styles.itemCopy}>
                  ...
                  <Text style={[styles.priceText, { color: colors.primary }]}>{money(item.regularPrice)}</Text>
                </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.productChoices}>
              {state.catalog.map((product) => (
                <Pressable key={product.id} onPress={() => chooseProduct(product)} style={({ pressed }) => [styles.productChoice, { borderColor: selectedProductId === product.id ? colors.primary : colors.border, backgroundColor: selectedProductId === product.id ? colors.primary + "12" : colors.background }, pressed && styles.pressed]}>
                  ...
                  <Text style={{ color: colors.primary, fontSize: 12, fontWeight: "700", marginTop: 6 }}>{money(product.regularPrice)}</Text>
                </Pressable>
              ))}
            </ScrollView>
            {selectedProduct ? (
              ...
              return (
                <View key={offer.id} style={[styles.offerItem, { borderColor: colors.border }]}>
                  <View style={styles.itemCopy}>
                    ...
                    <Text style={[styles.priceText, { color: colors.primary }]}>{money(offer.offerPrice)} <Text style={{ color: colors.muted, fontSize: 12, fontWeight: "400" }}>antes {money(offer.originalPrice)}</Text></Text>
...
function PrimaryButton({ label, onPress, color }: { label: string; onPress: () => void; color: string }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.primaryButton, { backgroundColor: color }, pressed && styles.pressed]}><Text style={styles.primaryButtonText}>{label}</Text></Pressable>;
}
